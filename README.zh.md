...
```bash
docker pull ghcr.io/etherfunlab/eros-engine:1.5.1
...
docker run --rm -p 8080:8080 --env-file .env \
  ghcr.io/etherfunlab/eros-engine:1.5.1 serve
```
...
核心流程很简单：创建人设会话，然后向 SSE 流式端点发送对话。另有一个只入队的备选端点（`POST /v2/comp/chat/{session_id}/message/async`），面向无法保持流式连接的 bot 式调用方——回复由后台 worker 生成，落库后从历史记录 / Realtime 读取，而不是走流式帧。引擎还提供历史记录、会话、画像及好感度路由。默认使用 Supabase JWT 鉴权，也可通过 `AuthValidator` 替换。具体路径、payload 与流式帧详见 [API 参考](docs/api-reference.zh.md)。
