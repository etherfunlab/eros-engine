...
- **會跟用戶一起長大的記憶層** —— 兩張 pgvector 表分工：一張存跨 session 的事實（profile 層），一張存當下 session 的回憶細節（relationship 層）。人格隔了幾週還記得你提過那家舊書店，不是把整段聊天記錄塞回 context 硬撐出來，是真的有層次的記憶結構。

- **確定性親密度，不靠 prompt 描述** —— 用戶跟人格之間的親密度是一條六維數值向量，每輪算一個 Δ 寫進去，不是在 system prompt 裡塞一段「你現在覺得比較親近用戶了」這種文字。數字會聽話；prompt 描述的情緒在長 session 裡會慢慢漂走。

- **順帶長出來的結構化用戶畫像** —— 每輪對話後台抽事實（城市、MBTI 暗示、感情價值觀、生活節奏 ⋯）合併進一份 JSONB 畫像，附帶加權訓練進度（training level）。想做陪伴、做日記、做情緒類產品、做任何「需要真的了解用戶」的功能都用得上——畫像是結構化的，不是黑箱向量，你想拿它驅動甚麼邏輯都行。
...
延伸閱讀：
- [架構](docs/architecture.zh.md)——crate 邊界、pipeline 階段、數據流
- [好感度模型](docs/affinity-model.zh.md)——6 個維度、EMA、時間衰退、關係標籤
- [Ghost 機制](docs/ghost-mechanics.zh.md)——評分公式 + 保護規則 + 實例計算
- [記憶層](docs/memory-layers.zh.md)——profile vs relationship、Voyage、pgvector 檢索
- [部署](docs/deploying.zh.md)——Fly.io、Docker compose、自帶 Postgres / IdP
- [API 參考](docs/api-reference.zh.md)——每個 `/comp/*` 端點
