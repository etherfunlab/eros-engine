...
CREATE TABLE companion_affinity (
...
CREATE INDEX idx_affinity_user ON companion_affinity(user_id);

CREATE TABLE companion_affinity_events (
    id           UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    affinity_id  UUID NOT NULL REFERENCES companion_affinity(id) ON DELETE CASCADE,
    event_type   TEXT NOT NULL CHECK (event_type IN ('message','ghost','gift','time_decay')),
    deltas       JSONB NOT NULL DEFAULT '{}',
    context      JSONB DEFAULT '{}',
    created_at   TIMESTAMPTZ NOT NULL DEFAULT now()
);
CREATE INDEX idx_affinity_events_affinity ON companion_affinity_events(affinity_id);
