...
- **好感度调参：**写入管线的旋钮——`AFFINITY_GRADE_UNIT`、`AFFINITY_NEG_FACTOR`、`AFFINITY_TIER_DECAY`、`AFFINITY_CROSS_PENALTY`、`AFFINITY_CROSS_PENALTY_START`、`AFFINITY_DELTA_THRESHOLD`、`AFFINITY_DEMO_BOOST`——全部可选；默认值复现 3.0 之前的每轮包络。见[好感度模型 → 写入管线](affinity-model.zh.md#写入管线好感度-30)。
