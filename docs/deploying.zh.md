...
- **噪声消除：** 一个 text 轮仍然取该 session 最近 20 条消息，但只注入其中 3–17 条，并且会把每条注入的 assistant 行的首句剥掉 —— 首句承载的是模型模仿自己的那部分语气（`唔` / `啊` / `嗯啊`），几乎不承载信息。剥完为空的 assistant 行整行丢弃，不会以空内容送出（部分供应商会拒绝）；user 行不剥。注入几条取决于这段关系的 `character_insights` 行填得有多满：十个字段里非空达到 7 个就只注入 3 条 —— 当前轮加上一轮的对话，这三条永远受保护 —— 每少一个字段多注入 2 条，行缺失或全空时注入 17 条。`CHAT_NOISE_CANCELLATION_DISABLED=1` 恢复改动前的形状：固定 20 条窗口，assistant 行整条注入。剥空过行或收窄过窗口的轮次会打一行 `INFO`，只含计数，不含消息内容。设计文档：[噪声消除](superpowers/specs/2026-09-02-echo-cancellation-plus-design.md)。
