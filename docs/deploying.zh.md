...
- **回声消除：** 每个 text 轮会注入该 session 最近 20 条消息，而模型会复读 —— 线上七个模型实测 0.57%–15.38% 的 assistant 轮命中，没有一个是零。留在窗口里的重复会推高下一份拷贝的概率，那份拷贝又被存下来、又被注入。因此在单轮注入的消息里，任何出现超过一次的非空字符串，其所有出现都不注入，唯一例外是当前轮自己的用户消息，它永远保留。被丢掉的位置不回补。比较是逐字节的，且不区分角色。系统提示里的 `[recent_conversation]` 块由独立查询渲染，不做去重，因此落在最近 3 轮里的重复仍会从那里到达模型。`CHAT_ECHO_CANCELLATION_DISABLED=1` 可以关掉；voice 路径有自己的窗口，不受影响。有丢弃的轮次会打一行 `INFO`，只含计数，不含消息内容。设计文档：[回声消除](superpowers/specs/2026-08-19-chat-echo-cancellation-design.md)。
...
- **异步对话队列（可选）：**`POST /v2/comp/chat/{session_id}/message/async`（见 [API 参考](api-reference.zh.md#post-v2compchatsession_idmessageasync)）把一轮对话交给后台 worker 处理，而不是流式返回——回复落在 `engine.chat_messages`，从历史接口或 Supabase Realtime 读取。这是第五个启动期 sweeper；`CHAT_QUEUE_DISABLED=1` 只停 worker（跟其他 sweeper 一致）——入队照样成功，只是会一直 `pending` 到重新开启。worker 每 `CHAT_QUEUE_TICK_SECS`（默认 5）秒醒一次，外加每次入队的即时唤醒；同时最多跑 `CHAT_QUEUE_CONCURRENCY`（默认 4）轮；认领超过 `CHAT_QUEUE_CLAIM_STALE_SECS`（默认 300）秒视为 worker 崩溃、可被重新认领；一轮最多重试 `CHAT_QUEUE_MAX_ATTEMPTS`（默认 3）次后标记为 `failed`；单次生成上限 `CHAT_QUEUE_GEN_TIMEOUT_SECS`（默认 300）秒；`CHAT_QUEUE_PENDING_CAP`（默认 20）是单会话入队深度上限，超过返回 `429 rate_limited`。队列顺序是严格的单会话 LIFO——会话里最新的消息最先处理，不会被丢弃或合并。设计文档：[async chat endpoint spec](superpowers/specs/2026-08-20-async-chat-endpoint-design.md)。
