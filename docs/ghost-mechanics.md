...
- It's **not** silent forever. `patience` is a derived endpoint (4.0): it recovers as soon as the judge reads a non-cold turn, and a high bond keeps it floored through absences; time-decay also softens `tension`. Eventually the persona will reply again to the next message.
