...
`|g|/4` 这个因子正是「正向裁决不会读成下跌」的来源。忽略规则微调，整项是 `ρ = g × (D_k·u − κ·φ(y)/4)`：括号内与档位无关，所以**结果的符号总是与裁决的符号一致**——档位决定幅度，不决定方向。而定额收税会让符号在 g1 与 g4 之间某处翻转，于是一次诚实的小推进反而掉分、更大的推进才涨分，判官全给正向的那一轮，表却在往下走。

双高互锁该在的地方仍然在。括号真正为负只发生在**自身第 5 档且对手线超过 ≈`0.761`**（默认参数下）——在那里**每一个**档位都净亏，且是齐平地亏。「不能既是挚友又是恋人」在顶点依然成立，只是不再对普通的中段回合开火。
...
实收跨线税非零的回合另记 `cross_penalty_charged`——惩罚改为按档位比例收之后，
一个回合到底交了多少税，已经无法只凭档位与存储分数反推。
