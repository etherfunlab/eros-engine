...
- **Affinity tuning:** the write pipeline's knobs — `AFFINITY_GRADE_UNIT`, `AFFINITY_NEG_FACTOR`, `AFFINITY_TIER_DECAY`, `AFFINITY_CROSS_PENALTY`, `AFFINITY_CROSS_PENALTY_START`, `AFFINITY_DELTA_THRESHOLD`, `AFFINITY_DEMO_BOOST`, plus the 3.1 scope-steering pair `AFFINITY_SCOPE_BOND_BOOST` / `AFFINITY_SCOPE_CHEM_LADDER` — are all optional; the defaults reproduce the pre-3.0 single-turn envelope, and the 3.1 pair reverts to it at `1.0` / `0,1,2,3,4`. See [Affinity model → Write pipeline](affinity-model.md#write-pipeline-affinity-30).
