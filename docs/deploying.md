...
- **Echo cancellation:** each text turn injects the session's last 20 messages, and models repeat themselves — measured across seven production models at 0.57%–15.38% of assistant turns, with no model at zero. A duplicate left in the window raises the probability of another copy, which is then stored and injected again. So within the messages one turn injects, any non-empty string occurring more than once is dropped in all of its occurrences, except the current turn's own user message, which is never dropped. Dropped slots are not backfilled. Comparison is byte-exact and role-blind. The `[recent_conversation]` block in the system prompt is rendered from its own query and is not deduplicated, so a duplicate inside the last 3 turn pairs still reaches the model there. `CHAT_ECHO_CANCELLATION_DISABLED=1` opts out; the voice path has its own window and is unaffected. Turns that dropped something log one `INFO` line with counts only — no message content. Design: [echo cancellation](superpowers/specs/2026-08-19-chat-echo-cancellation-design.md).
...
- **Async chat queue (optional):** `POST /v2/comp/chat/{session_id}/message/async` (see [API reference](api-reference.md#post-v2compchatsession_idmessageasync)) hands a chat turn to a background worker — the fifth boot-time sweeper — instead of streaming it; the reply lands in `engine.chat_messages` for pickup via history or Supabase Realtime. `CHAT_QUEUE_DISABLED=1` stops the worker only, mirroring the other sweepers — enqueues still succeed, they just sit `pending` until re-enabled. The worker wakes every `CHAT_QUEUE_TICK_SECS` (default 5, plus an immediate nudge per enqueue) and runs up to `CHAT_QUEUE_CONCURRENCY` (default 4) turns at once; a claim older than `CHAT_QUEUE_CLAIM_STALE_SECS` (default 300) is treated as a crashed worker and re-claimed; a turn gets up to `CHAT_QUEUE_MAX_ATTEMPTS` (default 3) attempts in total before it's marked `failed`; one generation is capped at `CHAT_QUEUE_GEN_TIMEOUT_SECS` (default 300); and `CHAT_QUEUE_PENDING_CAP` (default 20) is the per-session enqueue depth that returns `429 rate_limited`. Queue order is strict per-session LIFO — the newest message in a session goes first, and nothing is dropped or coalesced. Design: [async chat endpoint spec](superpowers/specs/2026-08-20-async-chat-endpoint-design.md).
...
- **Affinity for the frontend:** `GET /bff/v1/comp/affinity/{session_id}` returns the absolute vector refreshed at read time, and `.../event` returns the per-turn delta plus the post-turn state. `GET /bff/v1/comp/affinities/{user_id}` is the list form of the first — every companion's refreshed value in one paginated round trip, for a client rendering a conversation list. All three are JWT + ownership checked and always registered — the old `EXPOSE_AFFINITY_DEBUG` gate and the debug routes it hid are gone as of `v1.3.1`. See [migrating/affinity-4-0-v1-3-1.md](migrating/affinity-4-0-v1-3-1.md).
