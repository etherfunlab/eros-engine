...
- **Noise cancellation:** a text turn still fetches the session's last 20 messages, but injects only 3–17 of them, and strips the leading sentence off every assistant row it injects — that opening sentence carries the style the model copies from itself (`唔` / `啊` / `嗯啊`) and almost none of the information. An assistant row left empty by the strip is dropped rather than sent as empty content, which some providers reject; user rows are never stripped. How many rows are injected is a function of how completely that relationship's `character_insights` row is filled: 7 or more of its ten fields non-empty injects 3 rows in the steady state — the current turn, which is always kept, plus the previous exchange (the previous turn's user message and its assistant reply) — and each field below 7 adds 2 more, up to 17 rows when the row is absent or wholly empty. The previous exchange is not a flat two-row budget: it's the newest prior user row plus everything between it and the current turn, so a turn that left two assistant rows (e.g. a text reply and a separate image row) keeps three prior rows, not two. `CHAT_NOISE_CANCELLATION_DISABLED=1` restores the previous shape: a fixed 20-row window with assistant rows injected whole. Turns where the strip emptied a row or the ladder narrowed the window log one `INFO` line with counts only — no message content. Design: [echo cancellation plus](superpowers/specs/2026-09-02-echo-cancellation-plus-design.md).
