...
- **Affinity for the frontend:** `GET /bff/v1/comp/affinity/{session_id}` returns the absolute vector refreshed at read time, and `.../event` returns the per-turn delta plus the post-turn state. Both are JWT + ownership checked and always registered — the old `EXPOSE_AFFINITY_DEBUG` gate and the debug routes it hid are gone as of `v1.3.1`. See [migrating/affinity-4-0-v1-3-1.md](migrating/affinity-4-0-v1-3-1.md).
